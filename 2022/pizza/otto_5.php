...
//  print_r($clienti);
  ...
  for($e=0; $e<$n_epoche; $e++){
    ...
    if(time()-$zero > $max_time)
      break;
    for($try=0; $try< 3; $try++){
      $missing = array_diff($prob_array, $best_output, $checked_already);
      if(!empty($missing)){
        $output_to_check = $best_output;
        for($repeat =0;$repeat<3; $repeat++){
          $selection = array_rand($missing, 1);
          array_push($output_to_check, $missing[$selection]);
          array_push($checked_already, $missing[$selection]);
        }
