...
function read_file($file_name){
  ...
  return array($clienti, $gusti, $count_dislike);
}
...
//  array_count_values($clienti[$i]['dislike']);

  //print_r($output);
  $max_time = 0*60;
  $starting_point = $a[2];//ceil(count($clienti)/3)+1;
  unset($a);
  $slice = array_slice($clienti,0,$starting_point);
  $first_likes = array_merge(...array_column($slice, "like"));
  $first_dislikes = array_merge(...array_column($slice, "dislike"));
  $best_output = array_diff($first_likes, $first_dislikes);
  //print_r($best_output);
  $best_score = compute_score(array_values($best_output), $clienti);


  for($i=$starting_point+1; $i<count($clienti); $i++){
    $output = array_unique(array_merge($best_output, $clienti[$i]["like"]));
