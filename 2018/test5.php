...
//$input_files = array('2018/c_no_hurry');
...
while(min($auto_available)<$steps && count($corse)>0){
  ...
  print(">auto $v available at $auto_available[$v]\n");

  $chiave=-1;
	foreach(array_keys($corse) as $c_key){
			if(in_array($c_key,$invalid_couple[$v])){
          continue;
      }
			$c = $corse[$c_key];
    ...
    if($chiave==-1){
      print(">$chiave is NOT valid\n");
      $auto_available[$v] = $steps+1;
    }else{
      //print(">$chiave is valid\n");
      $time_to_start=($auto_available[$v] + distanza($auto_x[$v], $auto_y[$v],$corse[$chiave]['a'],$corse[$chiave]['b']));
  		...
  		if($time_corsa<$steps && $time_corsa<$corse[$chiave]['f']){
        print("==>Valid ride: $v, $chiave\n");
        ...
        $temp_f=$corse[$chiave]['f'];
        //print(">Invalid ride: $v, $chiave; $time_corsa < $steps; and  $time_corsa < $temp_f\n");
        if($temp_f==$steps){
          print(">skip vehicle at step ".$auto_available[$v]."\n");
          $auto_available[$v] = $steps+1;
        }
        array_push($invalid_couple[$v], $chiave);
      }
    }
